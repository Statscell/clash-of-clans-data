...
async function parseStats() {
  ...
  writeFileSync(
    './output/troopUpgradeStats.json',
    formatJson(outputItems, { type: 'space', size: 2 })
  );
}

const excludeFilterCharacter = ['TID_DRAGON_DUKE'];
...
function _parseStats(inputItems, type) {
  ...
  for (let i = 1; i < inputItems.length; i++) {
    ...
    if (
      (type !== TYPES.PETS && character.DisableProduction === true) ||
      (type !== TYPES.TROOPS && character.EnabledBySuperLicence === true) ||
      character.Deprecated === true ||
      character.IsSecondaryTroop === true ||
      (character.EnabledByCalendar === true && !excludeFilterCharacter.includes(character.TID))
    ) {
      validCharacter.name = '';
      continue;
    }

    const village = character.VillageType === 1 ? 'builderBase' : 'home';
    ...
    const upgradeResourceAndCost =
      type === TYPES.EQUIPMENT
        ? getResourceAndCost(character.UpgradeCosts, character.UpgradeResources)
        : null;
    const allowedCharacters =
      type === TYPES.EQUIPMENT ? getAllowedCharacters(character.AllowedCharacters) : [];
    ...
    if (type === TYPES.HEROES) {
      ...
      if (heroAltar) {
        ...
        unlockValues.time = formatUnlockTime(
          heroAltar.BuildTimeD,
          heroAltar.BuildTimeH,
          heroAltar.BuildTimeM,
          heroAltar.BuildTimeS
        );
      ...
      if (character.Name !== '' && character[productionBuildingType] !== '') {
        ...
        const _build = RAW_BUILDINGS.find((build) => {
          if (type === TYPES.EQUIPMENT) {
            ...
            return (
              build.BuildingLevel === buildingLevel &&
              build.ExportName.includes(`${productionBuilding[productionBuildingField]}`)
            );
          }
          return (
            build.ExportName ===
        ...
        const _buildLvl1 = RAW_BUILDINGS.find(
          (build) => build.ExportName === `${productionBuilding[productionBuildingField]}1`
        );
      ...
      if (!isNaN(character.RequiredBlacksmithLevel)) {
        const blRequirement = RAW_BUILDINGS.find((build) => {
          const buildingLevel = character[productionBuildingType];
          return (
            build.BuildingLevel === buildingLevel &&
            build.ExportName.includes(`${productionBuilding['Blacksmith']}`)
          );
        }).TownHallLevel;
        hallLevel = unlockValues.hall
          ? Math.max(unlockValues.hall, hallLevel)
    ...
    if (character.Name) {
      ...
      outputList.push({
        ...
        upgrade: {
          ...
          resources: upgradeResourceAndCost?.length ? [upgradeResourceAndCost] : []
        },
        allowedCharacters,
      ...
      const foundItem = outputList.find(
        (itm) => itm._name === validCharacter.name && itm.village === validCharacter.village
      );
  ...
  return outputList.map((itm) => {
    const levels = [];
    let previousLevel = 0;
    const maxHall = itm.village === 'home' ? maxTH : maxBH;
...
function formatUpdateTime(timeH, timeM) {
  const time = (isNaN(timeH) ? 0 : timeH * 60) + (isNaN(timeM) ? 0 : timeM);
  return time === 0 ? 0 : parseInt(time) * 60;
}

function formatUnlockTime(timeD, timeH, timeM, timeS) {
  return parseInt(
    (isNaN(timeD) ? 0 : timeD * 24 * 60 * 60) +
      (isNaN(timeH) ? 0 : timeH * 60 * 60) +
      (isNaN(timeM) ? 0 : timeM * 60) +
      (isNaN(timeS) ? 0 : timeS)
  );
}
...
const productionBuilding = {
  ...
  'Barracks': 'barracks_lvl',
  'Dark Barracks': 'darkBarracks_lvl',
  'Dark Elixir Barrack': 'darkBarracks_lvl',
  'Barrack2': 'adv_training_camp_lvl',
  'BB Barracks': 'adv_training_camp_lvl',

  'SiegeWorkshop': 'siegeWorkshop_lvl',
  'Siege Workshop': 'siegeWorkshop_lvl',
  'Spell Forge': 'spell_factory_lvl',
  'Spell Factory': 'spell_factory_lvl',

  'Mini Spell Factory': 'mini_spell_distillery_lvl',
  'Dark Spell Factory': 'mini_spell_distillery_lvl',
